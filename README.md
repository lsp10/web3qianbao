...
- 🔄 **自动锁定机制**：5 分钟无操作自动锁定钱包，清除内存中解密的助记词和密码，防范内存泄露与物理接触攻击。同时支持 Chrome MV3 后台重启时的锁定状态自动同步与恢复。
- 🔄 **一键重置钱包**：在锁定界面提供“重置钱包”入口，安全清除所有本地账户和加密密文，支持从零开始重新创建或导入其他钱包。
...
* **锁定与导出/重置**：
  * **导出**：在“设置”界面中，支持验证密码后安全地重新展示助记词。
  * **锁定**：点击“锁定”可以立即清除内存私钥并跳转回锁屏页面（5分钟无操作也会自动锁定）。
  * **重置**：当钱包处于锁定状态时，可在锁屏界面点击 **“重置并创建新钱包”**，通过二次确认清除本地存储中的所有加密 Vault，适合忘记密码或需重新初始化的情况。
...
1. **零密钥明文落盘**：
   ...
   - 加密密钥是根据用户密码加上随机盐值通过 PBKDF2 动态派生的，即使磁盘上的存储数据被恶意软件读取，在没有正确密码的情况下也极难破解。
...
3. **Chrome MV3 生命周期恢复机制 (Graceful State Restore)**：
   - 由于 Chrome MV3 中 Service Worker 可能会被浏览器自动挂起（Suspend），导致内存解密状态丢失恢复为 `LOCKED`。
   - 钱包前台 Popup 的 `loadDashboard` 引入了对此锁定状态的异常拦截与捕获：当检测到后台状态变更时，前台能自动识别并进行**优雅重定向**至解锁界面，保障钱包运行的稳定性与安全闭环。
4. **健壮的前后台连接握手 (Retry Mechanism)**：
   - 前台 Popup 初始化时，如遇后台 Service Worker 尚未就绪，前台会执行 **最多 3 次（每次间隔 500ms）** 的消息通道连接重试，保证在低性能设备或后台挂起冷启动时，消息收发百分之百稳定。
5. **健壮的单页应用导航 (Anti-Stacking & SPA Isolation)**：
   - 前台 UI 引入了高健壮性的 SPA 页面切换方法 `navigateTo`，每次切换均会完全清除其它子页面的激活类；
   - 并在 CSS 中注入安全防御机制：`.page:not(.active) { display: none !important; }`，彻底阻断了由于选择器优先级覆盖导致多层 UI 重叠或透光的渲染漏洞。
6. **EIP-6963 规范**：
